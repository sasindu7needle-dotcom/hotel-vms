...
<main class="registration-shell">
    ...
    <section class="registration-card" aria-labelledby="manual-registration-title">
        <div class="registration-heading">
            <span class="tagline no-margin">FRONT DESK</span>
            <h1 id="manual-registration-title" class="headline">Manual Registration<span class="dot">.</span></h1>
            <p>Register a walk-in visitor and securely capture their identity documents and face photo.</p>
        </div>
        @if(session('status'))
        ...
        <form method="POST" action="{{ route('visitor.manual.store') }}" enctype="multipart/form-data" class="registration-form">
            ...
            <div class="registration-grid">
                ...
                <div class="form-group"><label class="form-label-premium">Company</label><input class="form-control-premium @error('company') is-invalid @enderror" name="company" value="{{ old('company') }}" required placeholder="e.g. Manil Gems">@error('company')<span class="form-error-msg">{{ $message }}</span>@enderror</div>
            </div></fieldset>
            ...
            <div class="manual-flow-actions"><a class="manual-flow-cancel" href="{{ url('/') }}">Cancel</a><button class="btn btn-primary btn-large registration-next" type="submit">Register visitor <span>→</span></button></div>
        </form>
    </section>
...
category.addEventListener('change',()=>{const option=category.options[category.selectedIndex];if(option.dataset.fee!==undefined)fee.value=Number(option.dataset.fee).toFixed(2)});
