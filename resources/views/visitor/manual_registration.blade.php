...
<main class="registration-shell">
    ...
    <section class="registration-card" aria-labelledby="manual-registration-title">
        ...
        <form method="POST" action="{{ route('visitor.manual.store') }}" enctype="multipart/form-data" class="registration-form">
            ...
            <fieldset class="manual-flow-stage"><legend><span>1</span><strong>Visitor details</strong><small>Contact and work information</small></legend>
                <div class="registration-grid">
                    ...
                    <div class="form-group form-group-wide"><label class="form-label-premium">Email Address</label><input class="form-control-premium @error('email') is-invalid @enderror" type="email" name="email" value="{{ old('email') }}" required autocomplete="email" placeholder="e.g. visitor@example.com">@error('email')<span class="form-error-msg">{{ $message }}</span>@enderror</div>
            ...
            <fieldset class="manual-flow-stage"><legend><span>2</span><strong>Visit &amp; identity</strong><small>Select the identity type, upload it, then verify the extracted number</small></legend>
                <div class="registration-grid">
                    @if($exhibitorProfile)
                    ...
                    @else
                        @php($selectedCategory = $categories->firstWhere('id', (int) old('category_id')) ?? $categories->first())
                        <div class="form-group">
                            <label class="form-label-premium">Visitor Category</label>
                            <select class="form-control-premium @error('category_id') is-invalid @enderror" name="category_id" id="manual-category" required @disabled($categories->isEmpty())>
                                @forelse($categories as $category)
                                    <option value="{{ $category->id }}" data-fee="{{ $category->entrance_fee }}" @selected($selectedCategory?->is($category))>{{ $category->name }}</option>
                                @empty
                                    <option value="">No active visitor categories available</option>
                                @endforelse
                            </select>
                            @error('category_id')<span class="form-error-msg">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group"><label class="form-label-premium">Entrance Fee (LKR)</label><input id="manual-fee" class="form-control-premium" type="number" name="entrance_fee" min="0" step="0.01" value="{{ old('entrance_fee', $selectedCategory?->entrance_fee ?? '0.00') }}" required></div>
                    @endif
                ...
                @unless($exhibitorProfile)
                    <div class="manual-flow-upload-grid" style="margin-top:18px">
                        <label class="manual-flow-upload"><input id="manual-payment-slip" type="file" name="payment_slip" accept="image/jpeg,image/png,image/webp,application/pdf"><strong>Payment slip <span aria-hidden="true">(optional)</span></strong><small>Upload JPG, PNG, WebP, or PDF up to 10 MB</small></label>
                    </div>
                    @error('payment_slip')<span class="form-error-msg">{{ $message }}</span>@enderror
                @endunless
            ...
            <fieldset class="manual-flow-stage"><legend><span>3</span><strong>Upload face photo</strong><small>Available after identity verification</small></legend>
...
if(paymentSlip)paymentSlip.addEventListener('change',()=>{if(paymentSlip.files[0])paymentSlip.closest('label').querySelector('small').textContent=paymentSlip.files[0].name;});
